...
router.post("/add",async(req,res)=>{
    ...
    objpimg.mv("./public/product_img/" + objpimg.name,async(err)=>{
        if(err){
            ...
            var pprice = parseFloat(req.body.pprice); // Convert to float
            var pdiscount = parseFloat(req.body.pdiscount); // Convert to float

            var finalprice = pprice - (pprice * pdiscount / 100);
